...
_PKGS       := $(shell [ -f pkgs/$(HOSTNAME) ] && sh pkgs/$(HOSTNAME))
_PKGS_LOCAL := $(shell [ -f pkgs/$(HOSTNAME).local ] && sh pkgs/$(HOSTNAME).local)
PKGS        := $(sort $(_PKGS) $(_PKGS_LOCAL))
...
pkgs:
	@if [ -n '$(PKGS)' ]; then \
		...
		\
		_unlisted=$$(comm -23 <($(PACMAN) -Qeq) <(printf '%s\n' $(PKGS))); \
		if [ -n "$$_unlisted" ]; then \
